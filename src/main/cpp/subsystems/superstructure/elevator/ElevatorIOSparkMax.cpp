...
ElevatorIOSparkMax::ElevatorIOSparkMax(int busId, int id, SparkLowLevel::MotorType motorType, double kS, double kV, double kA,
    ...
    m_voltage(GetVoltage())
{
    ...
    m_motor->Configure(sparkMaxConfigs, SparkBase::ResetMode::kResetSafeParameters, SparkBase::PersistMode::kPersistParameters);
}

ElevatorIOSparkMax::ElevatorIOSparkMax(int busId, int id, SparkLowLevel::MotorType motorType, ElevatorConstants constants)
    : ElevatorIOSparkMax(busId, id, motorType, constants.kS, constants.kV, constants.kA,
...
{
    m_motor->GetClosedLoopController().SetReference(position.value(), SparkLowLevel::ControlType::kPosition);
}
...
void ElevatorIOSparkMax::SetLowerLimitEnable(bool enableLowerLimit)
{
    ...
    m_motor->Configure(config, SparkBase::ResetMode::kResetSafeParameters, SparkBase::PersistMode::kPersistParameters);
}

void ElevatorIOSparkMax::SetSpeed(double speed, bool overrideLowerLimit)
{
    m_motor->GetClosedLoopController().SetReference(speed + kG / frc::RobotController::GetInputVoltage(), SparkLowLevel::ControlType::kDutyCycle);
}
...
{
    m_motor->GetClosedLoopController().SetReference(voltage.value(), SparkLowLevel::ControlType::kVoltage);
}
