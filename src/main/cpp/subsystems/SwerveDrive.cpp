                                       ...
                                       PIDConstants rotationPID)
{
    ...
    AutoBuilder::configure(
        ...
        [this](const ChassisSpeeds &speeds, const DriveFeedforwards &feedforwards)
        {
            SetControl(pathplannerFollower
                           .WithSpeeds(speeds)
                           .WithWheelForceFeedforwardsX(feedforwards.robotRelativeForcesX)
                           .WithWheelForceFeedforwardsY(feedforwards.robotRelativeForcesY));
        },
        make_shared<PPHolonomicDriveController>(
            translationPID,
            rotationPID),
        std::move(config),
        []
        {
            auto const alliance = DriverStation::GetAlliance().value_or(DriverStation::Alliance::kBlue);
...
void SwerveDrive::followTrajectory(const SwerveSample &sample)
{
    ...
    meters_per_second_t yFeedback{choreo.yController->Calculate(
...
std::array<Rotation2d, 4> SwerveDrive::resetModuleOffsets(const std::array<frc::Rotation2d, 4>& targetOffsets)
{
    ...
    for (size_t i = 0; i < offsets.size(); ++i)
    {
        ...
        newOffset = (degree_t)frc::InputModulus(((degree_t)newOffset).value(), -180.0, 180.0);
