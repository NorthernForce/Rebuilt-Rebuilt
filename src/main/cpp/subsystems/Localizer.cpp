    ...
    : m_estimateTimeout(estimateTimeout)
{
    ...
    for (const auto& config : cameraConfigs)
    {
        ...
        auto camera =
            std::make_unique<AprilTagCamera>(std::move(io), config.displayName);
...
void Localizer::Periodic()
{
    ...
    for (auto& camera : m_cameras)
    {
        ...
        {
            m_estimatedPoses.emplace_back(poses[i], timestamps[i],
                                          camera->GetName());
...
std::vector<frc::Pose2d> Localizer::GetPosesFromCamera(
...
const std::vector<std::unique_ptr<AprilTagCamera>>& Localizer::GetCameras()
    const
{
    return m_cameras;
}
...
void Localizer::Log(const nfr::LogContext& log) const
{
    ...
    if (!m_estimatedPoses.empty())
    {
        ...
        log["latest_pose/rotation"]
            << latestPose.pose.Rotation().Degrees().value();
