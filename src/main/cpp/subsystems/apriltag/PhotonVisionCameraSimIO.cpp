...
PhotonVisionCameraSimIO::PhotonVisionCameraSimIO(const std::string& cameraName, 
    ...
    : m_cameraName(cameraName), m_cameraTransform(cameraTransform)
{
    ...
    else
    {
        ...
        m_visionSim->AddAprilTags(frc::AprilTagFieldLayout::LoadField(frc::AprilTagField::k2024Crescendo));
    }
    ...
    m_cameraSim = std::shared_ptr<photon::PhotonCameraSim>(
        new photon::PhotonCameraSim(m_cameraName, cameraProperties, m_cameraTransform));
...
void PhotonVisionCameraSimIO::UpdateInputs(AprilTagCameraInputs& inputs)
{
    ...
    for (const auto& result : results)
    {
        ...
        // For simulation, we can generate pose estimates directly from known AprilTag positions
...
std::shared_ptr<photon::PhotonCameraSim> PhotonVisionCameraSimIO::GetCameraSim() const
{
    return m_cameraSim;
}
