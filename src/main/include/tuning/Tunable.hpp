...
template <typename T>
Tunable<T>::Tunable(const std::string& key, const T& defaultValue,
      ...
      table_(nt::NetworkTableInstance::GetDefault().GetTable(tableName))
{
    ...
    try
    {
        ...
        else
        {
            ...
            nfr::logger[tableName + "/" + key]
                << static_cast<double>(defaultValue.value());
        }
    }
    catch (...)
    {
...
void Tunable<T>::InitializeNetworkTables() const
{
    ...
    {
        doublePublisher_ = std::make_unique<nt::DoublePublisher>(
            table_->GetDoubleTopic(key_).Publish());
        doubleSubscriber_ = std::make_unique<nt::DoubleSubscriber>(
            table_->GetDoubleTopic(key_).Subscribe(defaultValue_));
    }
    else if constexpr (std::is_same_v<T, int> || std::is_same_v<T, long>)
    {
        integerPublisher_ = std::make_unique<nt::IntegerPublisher>(
            table_->GetIntegerTopic(key_).Publish());
        integerSubscriber_ = std::make_unique<nt::IntegerSubscriber>(
            table_->GetIntegerTopic(key_).Subscribe(defaultValue_));
    }
    else if constexpr (std::is_same_v<T, bool>)
    {
        booleanPublisher_ = std::make_unique<nt::BooleanPublisher>(
            table_->GetBooleanTopic(key_).Publish());
        booleanSubscriber_ = std::make_unique<nt::BooleanSubscriber>(
            table_->GetBooleanTopic(key_).Subscribe(defaultValue_));
    }
    else
    {
        ...
        if constexpr (requires { defaultValue_.value(); })
        {
            doublePublisher_ = std::make_unique<nt::DoublePublisher>(
                table_->GetDoubleTopic(key_).Publish());
            doubleSubscriber_ = std::make_unique<nt::DoubleSubscriber>(
                table_->GetDoubleTopic(key_).Subscribe(
                    static_cast<double>(defaultValue_.value())));
        }
        else
        {
            ...
            doubleSubscriber_ = std::make_unique<nt::DoubleSubscriber>(
                table_->GetDoubleTopic(key_).Subscribe(
                    static_cast<double>(defaultValue_)));
        }
    }
}
...
void Tunable<T>::PublishValue() const
{
    ...
    else
    {
        ...
        if (doublePublisher_)
        {
            ...
            {
                doublePublisher_->Set(
                    static_cast<double>(currentValue_.value()));
            }
            else
            {
    ...
    else
    {
        ...
        if (doubleSubscriber_)
        {
            if constexpr (requires {
                              T{static_cast<typename T::value_type>(0.0)};
                          })
            {
                // Units type constructor
                return T{static_cast<typename T::value_type>(
                    doubleSubscriber_->Get())};
            }
            else
            {
                // Direct conversion
