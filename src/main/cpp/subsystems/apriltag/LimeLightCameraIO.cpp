...
LimeLightCameraIO::LimeLightCameraIO(const std::string& cameraName, const frc::Transform3d& cameraTransform)
...
void LimeLightCameraIO::UpdateInputs(AprilTagCameraInputs& inputs)
{
    ...
    auto estimate = LimelightHelpers::getBotPoseEstimate_wpiBlue_MegaTag2(m_cameraName);
...
void LimeLightCameraIO::Log(const nfr::LogContext& log) const
{
    ...
    log["pipeline_latency"] << LimelightHelpers::getLatency_Pipeline(m_cameraName);
    log["capture_latency"] << LimelightHelpers::getLatency_Capture(m_cameraName);
