...
namespace nfr
{
    ...
    LimeLightCameraIO::LimeLightCameraIO(
    ...
    void LimeLightCameraIO::UpdateInputs(AprilTagCameraInputs& inputs)
    {
        ...
        if (inputs.connected && estimate.tagCount > 0)
        {
            ...
            inputs.timestamps.push_back(
                units::second_t{estimate.timestampSeconds});
        }
    }
    ...
    void LimeLightCameraIO::SetReferencePose(const frc::Pose3d& pose)
    {
        ...
        LimelightHelpers::SetRobotOrientation(
            m_cameraName,
            pose.Rotation().Z().convert<units::degree>().value(),  // Yaw in
                                                                   // degrees
    ...
    void LimeLightCameraIO::Log(const nfr::LogContext& log) const
    {
        ...
        log["capture_latency"]
            << LimelightHelpers::getLatency_Capture(m_cameraName);
