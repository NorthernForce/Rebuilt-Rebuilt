...
/**
 * @brief A class to control a single stage of the elevator, including one motor
 * and one sensor at the bottom limit of the stage
 */
...
public:
     ...
     * @param motor A shared pointer to any implementation of the abstract class
     * ElevatorIO
     * @param sensor A shared pointer to any implementation of the abstract
     * class ElevatorSensorIO
     * @param errorTolerance A length value used to determine whether the
     * elevator is at an acceptable distance from its target (Recommended to be
    ...
    /**
     * @brief Sets the target position of the elevator and starts the motor
    ...
    /**
     * @brief Determines whether the elevator is within an acceptable distance
     * from its target using the error tolerance variable
    ...
    /**
     * @brief Determines whether the elevator is within an acceptable distance
     * from the supplied position using the error tolerance variable
     ...
     * This command sets the motor to move to a position using a PID loop and
     * finishes when the elevator has reached that position
     ...
     * This command does not end and keeps trying to move the elevator to the
     * position
     ...
     * This command disables the lower limit of the stage and moves it at the
     * supplied speed until the sensor is triggered
    ...
    /**
     * @brief Gets a command to run a quasistatic sysid routine in the forward
     * direction In a quasistatic routine the motor is sped up gradually, so the
     * voltage being used to accelerate the motor is effectively zero (hence:
     * static)
     */
    frc2::CommandPtr GetSysIdQuasistaticForward();

    /**
     * @brief Gets a command to run a quasistatic sysid routine in the reverse
     * direction In a quasistatic routine the motor is sped up gradually, so the
     * voltage being used to accelerate the motor is effectively zero (hence:
     * static)
     */
    ...
    /**
     * @brief Gets a command to run a dynamic sysid routine in the forward
     * direction In a dynamic routine a step-voltage is applied to the motor, so
     * acceleration behaviors can be determined
     */
    frc2::CommandPtr GetSysIdDynamicForward();

    /**
     * @brief Gets a command to run a dynamic sysid routine in the reverse
     * direction In a dynamic routine a step-voltage is applied to the motor, so
     * acceleration behaviors can be determined
     */
...
/**
 * @brief A class to control an elevator using any motor with a TalonFX
 * controller (Falcon500, KrakenX60, KrakenX44)
 */
class ElevatorIOTalonFX : public ElevatorIO
{
public:
    /**
     * @brief An expanded constructor with all values found in the
     * ElevatorConstants struct as separate parameters
     * @param id The CAN id of the motor
     * @param kS
     * @param kV
     * @param kA
     * @param kP The Proportional value of the PID loop used to control this
     * motor
     ...
     * @param kG
     * @param kCruiseVelocity The cruise velocity value for Phoenix6's
     * MotionMagic
     ...
     * @param kSprocketCircumference The circumference of the sprocket directly
     * attached to the elevator (How far the elevator moves with one turn of the
     * driving shaft)
     * @param kGearRatio The gear ratio from the output shaft of the motor to
     * the driving shaft
     * @param kInverted true if the motor should run in reverse, false if not
     * @param kUpperLimit The maximum mechanical height that the elevator stage
     * is capable of
     */
     ...
     * This constructor calls the expanded constructor with the values contained
     * in the "constants" parameter
     * @param id The CAN id of the motor
     * @param constants An ElevatorConstants struct containing all the constants
     * corresponding to that elevator stage
     */
    ElevatorIOTalonFX(int id, ElevatorConstants constants);

    /**
     * @brief Starts moving the motor towards the desired position using a
     * MotionMagicExpoVoltage loop
     ...
     * @param overrideLowerLimit Should only be true if the elevator is homing
     * and the lower limit of the elevator is being reset with a functioning
     * limit switch
     */
     ...
     * @param enableLowerLimit true if the lower soft limit should be enabled,
     * false if not
     */
     ...
     * This should only be used if the elevator is at the lower hard limit, NOT
     * the soft limit
     */
...
/**
 * @brief A class that inherits ElevatorIOTalonFX and is used to simulate a
 * TalonFX motor
 */
...
public:
     ...
     * @param dcMotorType The type of motor being used. Many motors are already
     * included under frc::DCMotor::[motor name]
     */
    ElevatorIOTalonFXSim(int id, ElevatorConstants constants,
                         frc::DCMotor dcMotorType);

    /**
     * @brief Updates the ElevatorSim with the voltages being supplied and the
     * TalonFXSimState with the motor's position and velocity
     */
    void UpdateSim();
...
/**
 * @brief A class that inherits ElevatorIOTalonFXS and is used to simulate a
...
public:
     ...
     * @param id The CAN id of the motor
     * @param motorType The selected motor type from the enum
...
/**
 * @brief A class that inherits ElevatorIOSparkMax and is used to simulate a
...
public:
     ...
     * @return true if the position of the elevator is within the error
     * tolerance range of the target position
     */
 ...
 * This command is mostly called in autonomous when it can only be called once
 * and the elevator must remain at that position until another command is called
 */
...
public:
    ...
    /**
     * @brief Normally this function would determine when the command is
     * supposed to end, but this command does not as it needs to keep moving the
     * elevator to the position until another command is called
     */
    bool IsFinished() override;
...
/**
 * @brief A command to move the elevator to its lower hard limit and reset its
 * position to zero
 */
class ElevatorHomingCommand
...
public:
    ...
    /**
     * @brief Disables the lower soft limit and begins moving the motor
     * downwards The Initialize function runs once when the command begins
     */
    void Initialize() override;
     ...
     * The IsFinished function determines if the command should stop running
     * @return true if the sensor has been triggered and the command should
     * stop, false if not
     */
    bool IsFinished() override;
...
public:
    /**
     * @param elevator A pointer to the elevator being used
     * @param speed A function that returns the speed value to move the elevator
     * at
     */
    ...
    /**
     * @brief Sets the speed of the motor to the return value of the speed()
     * function The Execute function runs every update until the command ends
     */
