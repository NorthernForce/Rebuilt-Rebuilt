...
namespace nfr
{
          ...
          m_visionSim(visionSim)
    {
        ...
        {
            m_visionSim =
                std::make_shared<photon::VisionSystemSim>("VisionSim");
        ...
        m_cameraSim = std::shared_ptr<photon::PhotonCameraSim>(
            new photon::PhotonCameraSim(&m_camera, cameraProperties));
    ...
    void PhotonVisionCameraSimIO::UpdateInputs(AprilTagCameraInputs& inputs)
    {
        ...
        for (const auto& result : results)
        {
            ...
            // For simulation, we can generate pose estimates directly from
            // known AprilTag positions
            if (result.HasTargets())
            {
                // Use the vision system's pose estimation
                // In a real implementation, this would use PhotonPoseEstimator
                // For simulation, we'll use the known robot pose with some
                // noise

                // Add timestamp
                inputs.timestamps.push_back(result.GetTimestamp());

                // Note: In a real simulation, we would compute the pose
                // estimate from the detected targets. For now, we'll leave this
                // empty and let the actual robot pose be set via
                // UpdateRobotPose
            }
        }
    ...
    std::shared_ptr<photon::PhotonCameraSim>
    PhotonVisionCameraSimIO::GetCameraSim() const
    {
        return m_cameraSim;
    }
