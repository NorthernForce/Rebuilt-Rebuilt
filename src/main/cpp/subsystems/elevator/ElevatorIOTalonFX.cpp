...
ElevatorIOTalonFX::ElevatorIOTalonFX(int id, double kS, double kV, double kA, double kP, double kI, double kD, double kG, turns_per_second_t kCruiseVelocity, turns_per_second_squared_t kAcceleration, turns_per_second_cubed_t kJerk, meter_t kSprocketCircumference, double kGearRatio, bool kInverted, meter_t kUpperLimit)
:
    ...
    m_voltageOut(0_V)
{
    ...
    talonFXConfigs.MotorOutput.Inverted = (kInverted ? ctre::phoenix6::signals::InvertedValue::Clockwise_Positive : ctre::phoenix6::signals::InvertedValue::CounterClockwise_Positive);
    ...
    talonFXConfigs.Feedback.SensorToMechanismRatio = (scalar_t)(kGearRatio / kSprocketCircumference).value();

    talonFXConfigs.SoftwareLimitSwitch.ForwardSoftLimitEnable = true;
    talonFXConfigs.SoftwareLimitSwitch.ForwardSoftLimitThreshold = (turn_t)kUpperLimit.value();
...
: ElevatorIOTalonFX(
    ...
    constants.kD,
    constants.kG,
    constants.kCruiseVelocity,
    constants.kAcceleration,
    constants.kJerk,
    constants.kSprocketCircumference,
...
{
    m_motor -> SetControl(m_motionMagicVoltage.WithPosition((turn_t)position.value()));
}
...
{
    m_motor -> SetControl(m_dutyCycleOut.WithOutput(speed + kG / frc::RobotController::GetInputVoltage()));
}
