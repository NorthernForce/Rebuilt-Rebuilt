...
namespace nfr
{
    ...
    public:
        ...
        void Log(const std::string& key, std::span<std::string> values) override;
    private:
        wpi::log::DataLog &logRef;
        std::unordered_map<std::string, std::variant<wpi::log::DoubleLogEntry,
            wpi::log::BooleanLogEntry, wpi::log::IntegerLogEntry, wpi::log::StringLogEntry,
            wpi::log::DoubleArrayLogEntry, wpi::log::BooleanArrayLogEntry, wpi::log::IntegerArrayLogEntry,
            wpi::log::StringArrayLogEntry>> entries;
    };
}