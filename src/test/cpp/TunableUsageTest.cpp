...
TEST_F(TunableUsageTest, DriveConstantsUsage)
{
    ...
    units::meters_per_second_t maxSpeed =
        nfr::TunableDriveConstants::kMaxTranslationSpeed;
    ...
    // Test array getters
    auto odometryStdDev =
        nfr::TunableDriveConstants::GetOdometryStandardDeviation();
...
TEST_F(TunableUsageTest, FunctionParametersWithTunables)
{
    ...
    auto testFunction = [](double speed, bool enabled,
                           units::meter_t length) -> double
    { return enabled ? speed * length.value() : 0.0; };

    double result = testFunction(nfr::TunableExampleConstants::kMotorSpeed,
...
TEST_F(TunableUsageTest, PIDControllerExample)
{
    ...
    // New output = 1.0 * 1.0 + 0.1 * 0.1 + 0.05 * 0.2 = 1.0 + 0.01 + 0.01
    // = 1.02
