...
void LEDSubsystem::ConfigureStateAnimation(LEDState state, const LEDAnimation& animation)
{
    m_stateAnimations[state] = animation;
}


void LEDSubsystem::SetStateColor(LEDState state, uint8_t r, uint8_t g, uint8_t b, 
...
void LEDSubsystem::SetStateRainbow(LEDState state, double brightness, uint32_t speed, 
                                  uint8_t startIndex, uint8_t count)
{
    LEDAnimation animation;
    animation.animationType = ctre::phoenix6::controls::AnimationType::RainbowAnimation;
...
void LEDSubsystem::SetStateColorFade(LEDState state, 
                                    uint8_t r1, uint8_t g1, uint8_t b1, 
                                    uint8_t r2, uint8_t g2, uint8_t b2, 
                                    uint32_t speed, uint8_t startIndex, uint8_t count)
{
    LEDAnimation animation;
    animation.animationType = ctre::phoenix6::controls::AnimationType::ColorFadeAnimation;
...
void LEDSubsystem::SetStateStrobe(LEDState state, 
                                 uint8_t r, uint8_t g, uint8_t b, 
                                 uint32_t speed, uint8_t startIndex, uint8_t count)
{
    LEDAnimation animation;
    animation.animationType = ctre::phoenix6::controls::AnimationType::StrobeAnimation;
    animation.color1 = {r, g, b};
...
void LEDSubsystem::SetStateChase(LEDState state, 
                               uint8_t r, uint8_t g, uint8_t b, 
                               uint32_t speed, uint8_t startIndex, uint8_t count)
{
    LEDAnimation animation;
    animation.animationType = ctre::phoenix6::controls::AnimationType::SingleFadeAnimation;
    animation.color1 = {r, g, b};
...
void LEDSubsystem::SetStateCustomAnimation(LEDState state, 
...
void LEDSubsystem::SetBrightness(double brightness)
{
    ...
    m_candle.GetConfigurator().Apply(
        ctre::phoenix6::configs::CANdleConfiguration().WithBrightnessScalar(m_brightness));
}
...
void LEDSubsystem::InitializeDefaultAnimations()
{
    ...
    // we can change these later for stuff if we gotta go something game specific
...
void LEDSubsystem::ApplyCurrentAnimation()
{
    ...
    if (animationIt != m_stateAnimations.end()) {
        ...
        switch (animation.animationType) {
            ...
            case ctre::phoenix6::controls::AnimationType::SetAll:
                m_candle.SetLEDs(
                    ...
                    0, animation.startIndex, animation.numLEDs);
                break;
            ...
            case ctre::phoenix6::controls::AnimationType::LarsonAnimation:
                // these animations require additional parameters that i don't got set up right now
                // for simplicity it will just default to SetAll animation for now
                m_candle.SetLEDs(
                    animation.color1[0], animation.color1[1], animation.color1[2],
                    0, animation.startIndex, animation.numLEDs);
                break;
        ...
        // if no animation is configured for this state justturn off the LEDs and dont use them
        TurnOff();
    }
}
