    ...
    : m_cameraName(cameraName), m_cameraTransform(cameraTransform)
{
    ...
    else
    {
        ...
        m_visionSim->AddAprilTags(frc::LoadAprilTagLayoutField(frc::AprilTagField::k2024Crescendo));
    }
    
    // Create camera simulation
    auto cameraProperties = photon::SimCameraProperties{};
    ...
    m_cameraSim = std::make_shared<photon::PhotonCameraSim>(
        m_cameraName, cameraProperties);
