...
ElevatorIOTalonFX::ElevatorIOTalonFX(int id, double kS, double kV, double kA,
    ...
    : m_motor(std::make_shared<ctre::phoenix6::hardware::TalonFX>(id)),
      ...
      m_isPresent(m_motor->IsConnected()), m_motionMagicVoltage(0_tr),
      m_dutyCycleOut(0), m_voltageOut(0_V) {
...
void ElevatorIOTalonFX::ResetPosition() { m_motor->SetPosition(0_tr); }

void ElevatorIOTalonFX::Stop() { m_motor->StopMotor(); }
...
bool ElevatorIOTalonFX::GetIsPresent() { return m_motor->IsConnected(); }