...
{
    auto pose =
        std::make_shared<frc::Pose2d>(1.0_m, 2.0_m, frc::Rotation2d{45.0_deg});
...
{
    auto pose =
        std::make_unique<frc::Pose2d>(1.0_m, 2.0_m, frc::Rotation2d{45.0_deg});
...
{
    frc::Pose3d pose{1.0_m, 2.0_m, 3.0_m,
                     frc::Rotation3d{0.1_rad, 0.2_rad, 0.3_rad}};
    EXPECT_NO_THROW({ nfr::logger["pose3d"] << pose; });
}

TEST_F(LoggingTest, LoggingPose3dSharedPtr)
{
    auto pose = std::make_shared<frc::Pose3d>(
        1.0_m, 2.0_m, 3.0_m, frc::Rotation3d{0.1_rad, 0.2_rad, 0.3_rad});
    EXPECT_NO_THROW({ nfr::logger["pose3d_shared"] << pose; });
}

TEST_F(LoggingTest, LoggingPose3dUniquePtr)
{
    auto pose = std::make_unique<frc::Pose3d>(
        1.0_m, 2.0_m, 3.0_m, frc::Rotation3d{0.1_rad, 0.2_rad, 0.3_rad});
...
{
    auto translation =
        std::make_shared<frc::Translation3d>(1.5_m, 2.5_m, 3.5_m);
...
{
    auto rotation =
        std::make_shared<frc::Rotation3d>(0.1_rad, 0.2_rad, 0.3_rad);
    EXPECT_NO_THROW({ nfr::logger["rotation3d_shared"] << rotation; });
}

TEST_F(LoggingTest, LoggingTransform2dDirect)
{
    frc::Transform2d transform{frc::Translation2d{1.0_m, 2.0_m},
                               frc::Rotation2d{45.0_deg}};
    EXPECT_NO_THROW({ nfr::logger["transform2d"] << transform; });
}

TEST_F(LoggingTest, LoggingTransform2dSharedPtr)
{
    auto transform = std::make_shared<frc::Transform2d>(
        frc::Translation2d{1.0_m, 2.0_m}, frc::Rotation2d{45.0_deg});
    EXPECT_NO_THROW({ nfr::logger["transform2d_shared"] << transform; });
}

TEST_F(LoggingTest, LoggingTransform3dDirect)
{
    frc::Transform3d transform{frc::Translation3d{1.0_m, 2.0_m, 3.0_m},
                               frc::Rotation3d{0.1_rad, 0.2_rad, 0.3_rad}};
...
{
    auto transform = std::make_shared<frc::Transform3d>(
        frc::Translation3d{1.0_m, 2.0_m, 3.0_m},
        frc::Rotation3d{0.1_rad, 0.2_rad, 0.3_rad});
...
{
    auto speeds =
        std::make_shared<frc::ChassisSpeeds>(1.0_mps, 2.0_mps, 0.5_rad_per_s);
...
{
    auto speeds =
        std::make_unique<frc::ChassisSpeeds>(1.0_mps, 2.0_mps, 0.5_rad_per_s);
...
{
    auto state = std::make_shared<frc::SwerveModuleState>(
        2.0_mps, frc::Rotation2d{45.0_deg});
...
{
    auto position = std::make_shared<frc::SwerveModulePosition>(
        1.5_m, frc::Rotation2d{30.0_deg});
...
TEST_F(LoggingTest, LoggingUnitsAngularVelocitySharedPtr)
{
    auto angularVelocity =
        std::make_shared<units::radians_per_second_t>(3.14_rad_per_s);
    EXPECT_NO_THROW(
        { nfr::logger["angular_velocity_shared"] << angularVelocity; });
}
...
{
    auto acceleration =
        std::make_shared<units::meters_per_second_squared_t>(9.81_mps_sq);
...
TEST_F(LoggingTest, LoggingMixedPointerTypes)
{
    ...
    auto sharedPose =
        std::make_shared<frc::Pose2d>(2.0_m, 3.0_m, frc::Rotation2d{90.0_deg});
    auto uniquePose =
        std::make_unique<frc::Pose2d>(3.0_m, 4.0_m, frc::Rotation2d{135.0_deg});
